    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if len(categories_in_cart) == 1:
                ...
                ms_part_1 = _('The order has been successfully formed. Your order number:')
                ms_part_2 = _('Thanks!')
                ...
                ms_part_1 = _('Error: All items in your cart must belong to the same category! Categories in your order:')
                categories_list = ', '.join(
                    [category.name for category in categories_in_cart])
                messages.error(request,
                               f'{ms_part_1} {categories_list}.')
        else:
            messages.error(request, _('Error when placing or editing an order, check the form data!'))
    else:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if len(categories_in_cart) == 1:
                ...
                ms_part_1 = _('The draft order has been successfully formed. Number:')
            ...
            messages.error(request,_('There was an error while placing or editing the draft order, check the form data!'))
    else:
...
def get_template_order_header_options(request):
    ...
    try:
        company = Company.objects.get(users_companies__id=user_company_id,is_deleted=False)
...
def get_stock_options(request):
    ...
    try:
        technique_queryset = Technique.objects.filter(company=company, is_deleted=False)
        technique_options = {str(technique.id): technique.serial_number for
                             technique in technique_queryset}
    except Technique.DoesNotExist:
        technique_options = {}
...
def orders(request):
    ...
    orders = Order.objects.select_related('user', 'template', 'company',
                                          'stock',
                                          'signatory_of_documents').values('status','id','created','company__name','first_and_last_name','formed')
    if query_search:
    ...
    products_in_order = OrderItem.objects.filter(
        ...
        'order', 'product', 'product__name', 'product__description','product__category__name', 'price',
        'pre_quantity',
        'ord_quantity',
    )
...
def edit_order(request, order_id):
    ...
    if request.method == 'POST':
        ...
        order_item_formset = OrderItemFormSet(request.POST, instance=order, form_kwargs={'request': request})
        ...
        order_item_formset = OrderItemFormSet(instance=order, form_kwargs={'request': request})
...
def excel_create(request, order_id):
    ...
    # Записуємо шапку з інформацією про замовлення.
    ...
    # Якщо замовлення підтверджено, курс залишається на дату останнього оновлення
    # (підтвердження замовлення); інакше: курс встановлюється на поточну дату.


    if order.status == 'order':
        confirmation_date = updated
        euro_rate = order.rate

    elif order.status == 'draft':
        confirmation_date = datetime.now()
        euro_rate = get_euro_exchange_rate(format_date(confirmation_date))
    ...
    fields = [
        ...
        (str(_('VIN code')), order.VIN_code),
        (str(_('Address')), order.address),
    ...
    order_item_columns = ['№',str(_('Category')) ,str(_('Product')) ,str(_('Description')) ,str(_('Price (€)')),
                          str(_('Pre quantity')),
                          str(_('Pre cost (€) without VAT')),
                          str(_('Pre cost (€) with VAT')),
                          str(_('Pre cost (₴) without VAT')),
                          str(_('Pre cost (₴) with VAT')), str(_('Ord quantity')),
    ...
    for index, order_item in enumerate(order_items, start=1):
        ...
        ws.write(row_num, 1, order_item.product.category.name, font_style)
        ws.write(row_num, 2, order_item.product.name, font_style)
        ws.write(row_num, 3, order_item.product.description, font_style)
        ws.write(row_num, 4, number_format(order_item.price, decimal_pos=2), font_style)
        ws.write(row_num, 5, number_format(order_item.pre_quantity), font_style)
        ws.write(row_num, 6,
                 number_format(order_item.price * order_item.pre_quantity,
                               decimal_pos=2), font_style)
        ws.write(row_num, 7, number_format(
            order_item.price * order_item.pre_quantity * vat_rate, decimal_pos=2),
                 font_style)
        ws.write(row_num, 8, number_format(
            order_item.price * order_item.pre_quantity * euro_rate, decimal_pos=2),
                 font_style)
        ws.write(row_num, 9, number_format(
            order_item.price * order_item.pre_quantity * vat_rate * euro_rate,
            decimal_pos=2),
                 font_style)
        ws.write(row_num, 10, number_format(order_item.ord_quantity), font_style)
        ws.write(row_num, 11, number_format(order_item.price * order_item.ord_quantity, decimal_pos=2),
                 font_style)
        ws.write(row_num, 12, number_format(
            order_item.price * order_item.ord_quantity * vat_rate, decimal_pos=2),
                 font_style)
        ws.write(row_num, 13, number_format(
            order_item.price * order_item.ord_quantity * euro_rate, decimal_pos=2),
                 font_style)
        ws.write(row_num, 14, number_format(
            order_item.price * order_item.ord_quantity * vat_rate * euro_rate,
            decimal_pos=2),
                 font_style)

        total_pre_amount += order_item.price * order_item.pre_quantity
        total_pre_amount_with_vat += round(order_item.price * order_item.pre_quantity * vat_rate,2)
        total_pre_amount_ua += round(order_item.price * order_item.pre_quantity * euro_rate,2)
        total_pre_amount_with_vat_ua += round(order_item.price * order_item.pre_quantity * vat_rate * euro_rate,2)
        ...
        total_ord_amount_ua += round(
            order_item.price * order_item.ord_quantity * euro_rate, 2)
    ...
    ws.write(row_num + 2, 7, number_format(total_pre_amount_with_vat), font_style)
    ws.write(row_num + 2, 8, number_format(total_pre_amount_ua), font_style)
    ws.write(row_num + 2, 9, number_format(total_pre_amount_with_vat_ua), font_style)

    ws.write(row_num + 2, 11, number_format(total_ord_amount), font_style)
    ws.write(row_num + 2, 12, number_format(total_ord_amount_with_vat), font_style)
    ws.write(row_num + 2, 13, number_format(total_ord_amount_ua), font_style)
    ws.write(row_num + 2, 14, number_format(total_ord_amount_with_vat_ua), font_style)
